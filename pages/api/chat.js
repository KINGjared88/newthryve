...
const ChatbotWidget = () => {
  ...
  const [messages, setMessages] = useState([
    {
      ...
      type: "leadmagnet", // Mark this message for special rendering
    },
  ]);
  ...
  // Input focus on open or after sending
  useEffect(() => {
  ...
  // Handle send message (user chat, not for lead magnet info)
  ...
  const handleLeadMagnet = (choice) => {
    if (choice === "yes") {
      ...
      // Remove lead magnet from future logic but leave in chat
      setMessages((prev) =>
        prev.map((msg) =>
          msg.type === "leadmagnet"
            ? {
                ...msg,
                content: `${msg.content}\n\n(No problem! Ask me anything about credit repair or our services.)`,
              }
            : msg
        )
      );
  ...
  // Submit info for free guide (trigger Zapier)
